...
RegisterTableModel::RegisterTableModel(QObject *parent) : QSqlTableModel(parent)
{
    ...
    header<<QObject::trUtf8("卡号")<<QObject::trUtf8("用户编号")
             <<QObject::trUtf8("时间")<<QObject::trUtf8("学号")<<QObject::trUtf8("手机号");
}
...
int RegisterTableModel::findstunum(QString &tagId){
    int row;
    int stunum;
    for(row=0; row<rowCount(); row++){
        if(data(index(row, 0)) == tagId){
            qDebug()<<data(index(row,3));
           //stunum = (int)data(index(row,3)).toString().right(1).toLatin1().data();
           stunum = data(index(row,3)).toInt();
           return stunum % 2;
        }
    }
    return 0;
 ...
 * 向注册表中插入数据
 */
bool RegisterTableModel::addRecord(QString &cardId,QString &personId, QString &time, QString &stunum,QString &remark)
{
    QSqlQuery query;
    QString str;
    str  = tr("insert into ") + tableName + tr(" values( \"%1\" , \"%2\" , \"%3\" , \"%4\" , \"%5\") ")
            .arg(cardId).arg(personId).arg(time).arg(stunum).arg(remark);
    qDebug()<<"Sql: " << str.toUtf8().data();
