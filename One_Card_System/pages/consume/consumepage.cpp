...
void ConsumePage::on_cardIdReceived(QString tagId)
{
    ...
    WriteOffTableModel *writeOffTable = new WriteOffTableModel(this);//判断是否挂失
    writeOffTable->bindTable();
    writeOffTable->findRecord(tagId);

    if(writeOffTable->findguashi(tagId) != -1){
        {
            messageBox->setText("该卡已经挂失,不能使用,谢谢!");
            messageBox->exec();
...
void ConsumePage::writeauthentication()
{
    ...
    int num = registerTableModel->findstunum(t);

    if(num == 1)  //学号为奇数从2号扇区开始
        buffer[1] = 0x0A;   // 绝对块号
    else          //学号为偶数从3号扇区开始
        buffer[1] = 0x0C;
    for(int i = 2 ; i < 8 ; i ++)
