    ...
    QDialog(parent), ui(new Ui::WriteOffPerson)
{
    ...
    connect(ui->push,SIGNAL(clicked(bool)),this,SLOT(on_jiegua_clicked()));
    connect(ui->push_2,SIGNAL(clicked(bool)),this,SLOT(on_guashi_clicked()));
}
...
void WriteOffPerson::on_btn_LogOff_clicked()
{
    ...
    if(row >= 0)
    {
        ...
        {
            writeOffTable->updateRecords(cardId,currentTime,logOffMark,guashi);
        }else {
            writeOffTable->addRecords(cardId,currentTime,logOffMark,guashi);
        }
...
//挂失按钮点击事件
void WriteOffPerson::on_guashi_clicked()
{
    QString cardId = ui->lineEdit_CardId->text();
    QString logOffMark = ui->textEdit_Mark->toPlainText();
    QString currentTime = CurrentDateTime();
    ...
    {
        QString guashi="1";
        qDebug() << writeOffTable->findRecord(cardId);
        if(writeOffTable->findRecord(cardId) >= 0)
        {
            writeOffTable->updateRecords(cardId,currentTime,logOffMark,guashi);
        }else {
...
//解挂按钮点击事件
void WriteOffPerson::on_jiegua_clicked()
{
    QString cardId = ui->lineEdit_CardId->text();
    QString logOffMark = ui->textEdit_Mark->toPlainText();
    QString currentTime = CurrentDateTime();
    ...
    int row = writeOffTable->findRecord(cardId);
    if(row >= 0)
    {
        QString guashi="0";
        if(writeOffTable->findRecord(cardId) >= 0)
